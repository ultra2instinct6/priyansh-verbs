/* =====================================================================
 * abc.js â€” Sound Ladder (ABC Side Game)
 * ---------------------------------------------------------------------
 ...
 *
 * Sections:
 *   1. CONFIG     â€” letters, words, sentences, tunables.
 *   2. STATE      â€” runtime per-session state.
 *   3. STORAGE    â€” persistent progress + parent-mode stats.
 *   4. AUDIO      â€” speech synthesis helper (sounds + words).
 *   5. UI / DOM   â€” overlay scaffold.
 *   6. SCREENS    â€” home, level, game, reward, parent, review, rewards.
 *   7. GAMEPLAY   â€” round generation, answer handling, feedback.
 *   8. PUBLIC API â€” openAbcGame / closeAbcGame on window.
 * ===================================================================== */
(function () {
  "use strict";
  ...
  // phoneme unambiguous: "/p/ â€” as in pat".
  const LETTERS = {
  ...
  const WORD_GLOSSES = {
    ...
    // Rotating cast â€” keeps stories from feeling same-y.
    Mia:  { emoji: "\uD83D\uDC67\uD83C\uDFFD" },
    Ben:  { emoji: "\uD83D\uDC66\uD83C\uDFFC" },
    Lila: { emoji: "\uD83D\uDC67\uD83C\uDFFB" },
    Kai:  { emoji: "\uD83D\uDC66\uD83C\uDFFE" },
  ...
  // 5 progressive levels. Each adds 1â€“2 new letters + new decodable words.
  const LEVELS = [
    {
      id: "L1",
    ...
    },
    {
      ...
      theme: "camp", themeIcon: "â›º",
      story: [
        "Lila and Ben pack a big bag for camp.",
        "A bed, a bat, a kit, a log â€” it all fits!",
    ...
    },
    {
      ...
      theme: "mountain", themeIcon: "â›°ï¸",
      letters: ["a","e","i","o","u","sh","ch","th","s","t","p","n","r","l","k","b","d","m","f","h","g","w","ee","ai","oa"],
      bossMode: true,
      bossMeter: true,
      story: [
        "Ch.1 â€” Mia and Kai took a boat to the lake.",
        "Ch.2 â€” They saw a big green frog by a leaf.",
        "Ch.3 â€” The frog hopped! It made a splash.",
        "Ch.4 â€” Mia said, \"That was the BEST day!\""
      ],
  ...
  // -------------------------------------------------------------------
  // 4. AUDIO â€” SpeechSynthesis helper
  // -------------------------------------------------------------------
  function speak(text, opts) {
    ...
    } catch (_) { /* speech not available â€” silent fallback */ }
  }
  ...
  function pickWeighted(arr, kind) {
    ...
    const weights = arr.map(item => {
      const t = stats[item];
      if (!t) return 1.5;                       // unseen â†’ moderate boost
      const tot = t.correct + t.wrong;
      if (tot === 0) return 1.5;
      const accuracy = t.correct / tot;
      if (accuracy < 0.5) return 3.0;            // weak â†’ strong boost
      if (accuracy < 0.8) return 2.0;            // shaky â†’ medium boost
      return 1.0;                                // mastered â†’ baseline
    });
  ...
  function renderLevel(body) {
    ...
    const cards = LEVELS.map((L, idx) => {
      ...
      // Unlock rule: L1 always open; later levels open once previous level has â‰¥1 star
  ...
  function generateRound(level) {
    ...
    // ---- Phonemic awareness: rhyme step (only when level has â‰¥2 same-suffix words)
    ...
    const _isEarly = _earlyLvlIdx >= 0 && _earlyLvlIdx <= 5; // L1â€“L6
